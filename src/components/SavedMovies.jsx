...
const SavedMovies = () => {
  ...
  const deleteShow = async (passedID) => {
    try {
      const data = movies.filter((item) => item.id !== passedID);
  ...
  return (
    <>
      <h2 className="text-white font-bold md:text-xl pd-4">My Movies</h2>
      <div className="relative flex items-center group">
        ...
        >
          {movies.map((item, id) => (
            <div
              key={id}
              className="w-[160px] sm:w-[200px] md:w-[240px] lg:w-[280] inline-block cursor-pointer relative p-2"
            >
              <img
